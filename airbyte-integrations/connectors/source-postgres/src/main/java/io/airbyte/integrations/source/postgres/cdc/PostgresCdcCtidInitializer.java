...
public class PostgresCdcCtidInitializer {
        ...
        try {
            ...
            if (isDebugMode(sourceConfig) && !PostgresUtils.shouldFlushAfterSync(sourceConfig)) {
                throw new ConfigErrorException(
                        "WARNING: The config indicates that we are clearing the WAL while reading data. This will mutate the WAL"
                                +
            ...
            final boolean savedOffsetAfterReplicationSlotLSN = postgresDebeziumStateUtil
                    .isSavedOffsetAfterReplicationSlotLSN(
                            // We can assume that there will be only 1 replication slot cause before the
                            // sync starts for
                            // Postgres CDC,
                            // we run all the check operations and one of the check validates that the
            ...
            if (!savedOffsetAfterReplicationSlotLSN) {
                ...
                if (!sourceConfig.get("replication_method").has(INVALID_CDC_CURSOR_POSITION_PROPERTY)
                        || sourceConfig.get("replication_method").get(
            ...
            final CtidStreams ctidStreams = PostgresCdcCtidUtils.streamsToSyncViaCtid(stateManager.getCdcStateManager(),
                    catalog,
            ...
            if (!ctidStreams.streamsForCtidSync().isEmpty()) {
                ...
                final List<ConfiguredAirbyteStream> finalListOfStreamsToBeSyncedViaCtid = streamsUnderVacuum.isEmpty()
                        ? ctidStreams.streamsForCtidSync()
                        : ctidStreams.streamsForCtidSync().stream()
                                .filter(c -> !streamsUnderVacuum
                                        .contains(AirbyteStreamNameNamespacePair.fromConfiguredAirbyteSteam(c)))
                                .toList();
                ...
                final FileNodeHandler fileNodeHandler = PostgresQueryUtils.fileNodeForStreams(database,
                        ...
                        quoteString);
                final CtidStateManager ctidStateManager = new CtidGlobalStateManager(ctidStreams, fileNodeHandler,
                        stateToBeUsed, catalog);
                final CtidPostgresSourceOperations ctidPostgresSourceOperations = new CtidPostgresSourceOperations(
                        Optional.of(new PostgresCdcConnectorMetadataInjector(emittedAt.toString(),
                                io.airbyte.cdk.db.PostgresUtils.getLsn(database).asLong())));
                final Map<io.airbyte.protocol.models.AirbyteStreamNameNamespacePair, TableBlockSize> tableBlockSizes = PostgresQueryUtils
                        .getTableBlockSizeForStreams(
                                database,
                                finalListOfStreamsToBeSyncedViaCtid,
                                quoteString);

                final Map<io.airbyte.protocol.models.AirbyteStreamNameNamespacePair, Integer> tablesMaxTuple = CtidUtils
                        .isTidRangeScanCapableDBServer(database) ? null
                                : PostgresQueryUtils.getTableMaxTupleForStreams(database,
                                        finalListOfStreamsToBeSyncedViaCtid, quoteString);
            ...
            // Attempt to advance LSN past the target position. For versions of Postgres
            // before PG15, this
            ...
            // ADD JSONB cols
            final Map<String, List<String>> jsonbColumnsPerTable = getJsonbColumnsFromCatalog(catalog);
            final var eventConverter = new RelationalDbDebeziumEventConverter(
                    new PostgresCdcConnectorMetadataInjector(), emittedAt, jsonbColumnsPerTable);

            final Supplier<AutoCloseableIterator<AirbyteMessage>> incrementalIteratorSupplier = () -> handler
                    .getIncrementalIterators(
                            propertiesManager, eventConverter, new PostgresCdcSavedInfoFetcher(stateToBeUsed),
            ...
            if (streamsUnderVacuum.isEmpty()) {
                // This starts processing the WAL as soon as initial sync is complete, this is a
                // bit different from
                // the current cdc syncs.
                // We finish the current CDC once the initial snapshot is complete and the next
                // sync starts
                // processing the WAL
                return Stream
                        .of(initialSyncCtidIterators,
    ...
    private static Map<String, List<String>> getJsonbColumnsFromCatalog(final ConfiguredAirbyteCatalog catalog) {
        return catalog.getStreams().stream()
                .filter(s -> s.getSyncMode() == SyncMode.INCREMENTAL)
                .map(ConfiguredAirbyteStream::getStream)
                .map(s -> {
                    final JsonNode streamProperties = s.getJsonSchema().get("properties");
                    final Iterable<String> iter = () -> streamProperties.fieldNames();
                    final List<String> jsonbColumns = StreamSupport.stream(iter.spliterator(), false)
                            .filter(col -> {
                                final JsonNode columnProperties = streamProperties.get(col);
                                // jsonb is column type unknown currently, which doesn't set a type
                                // need a better way to handle this long term!
                                return !columnProperties.has("type");
                            }).collect(Collectors.toList());
                    LOGGER.info("Table {} jsonb columns: {}", s.getName(), String.join(", ", jsonbColumns));
                    return new SimpleEntry<String, List<String>>(s.getName(), jsonbColumns);
